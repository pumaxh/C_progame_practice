/*
 * Estimate necessary solar collecting area size for a paticular type of
...
int main(void)
{
    int heat_deg_days,  /* average for coldest month */
        solar_insol,    /* average daily solar radiation
                           per ft^2 for coldest month */
        coldest_mon,   /* coldest month: number in 1..12 */
        heating_req,   /* BTU/degree day ft^2 requirement
                          for given type of construction */
    ...
    double floor_space,  /* ft^2 */
           ...
           energy_resrc; /* BTUs heat obtained form 1 ft^2
    ...
    /* Get average heating degree days for coldest month form file */
    ...
    /* Get corresponding average daily colar insolation form other file */
    solar_file = fopen("solar.txt", "r");
    solar_insol = nth_item(solar_file, coldest_mon);
    fclose(solar_file);

    /* Get from user specifice of this house */
    ...
    /* Project collection area needed */
...
/*
 * Finds and returns the nth interger in a file.
 * Pre: data_file accesses a file of at least n intergers(n >= 1)
 */
